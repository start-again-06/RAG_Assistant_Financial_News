...
Retrieval-Augmented Generation (RAG) combines document retrieval with language generation to produce answers grounded in external knowledge, improving accuracy and reducing hallucinations.
...
Retrieval-Augmented Generation can be represented mathematically by combining *document retrieval probability* and *language model generation probability*.

### Query Embedding

A user query $\( q \)$ is converted into a vector embedding:
...
- $\( e_q \)$ is the vector representation of the query

---

### Document Similarity

Each document $\( d_i \)$ is also embedded into vector space:

$\[e_{d_i} = f(d_i)\]$

Similarity between query and document is computed using **cosine similarity**:

$\[sim(q, d_i) = \frac{e_q \cdot e_{d_i}} {\|e_q\|\|e_{d_i}\|}\]$

The system retrieves the *top-k most relevant documents*:

$\[D_k = \text{TopK}(sim(q, d_i))\]$

---

### Retrieval Probability

The retriever estimates the probability of selecting document $\( d \)$ given query $\( q \)$:
...
The final response $\( y \)$ is generated by conditioning the language model on retrieved documents.

$\[P(y|q) = \sum_{d \in D_k} P(y|q,d)P(d|q)\]$

where:

- $\( P(d|q) \)$ is the retrieval probability
- $\( P(y|q,d) \)$ is the language model generation probability
...
This ensures the model produces answers that are **grounded in retrieved financial news documents**.
